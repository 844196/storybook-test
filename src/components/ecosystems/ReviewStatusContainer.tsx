...
export const ReviewStatusContainer: React.FC<ReviewStatusContainerProps> = ({ prUrl, rowDom, loginUsername }) => {
  // TODO 多分ここより上層の責務
  const [isDisplayDefault] = useExtensionConfig('isDisplayDefault');
  const [enableBackgroundColor] = useExtensionConfig('enableBackgroundColor');
  const [cacheMaxAge] = useExtensionConfig('cacheMaxAge');
  if (!isDisplayDefault || enableBackgroundColor === undefined || cacheMaxAge === undefined) {
    return null;
  }
